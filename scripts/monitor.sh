...
# Header
if [ ! -f "$LOGFILE" ]; then
  echo "timestamp,mem_total_kb,mem_free_kb,mem_cached_kb,mem_available_kb,swap_used_kb,oc_gateway_rss_kb,oc_parent_rss_kb,proot_cpu_pct,gw_cpu_pct,disk_free_mb,battery_pct,battery_temp" > "$LOGFILE"
fi
...
while true; do
  ...
  # CPU (snapshot)
  PROOT_CPU=$(ps aux 2>/dev/null | grep 'proot --link' | grep -v grep | awk '{print $3}' | head -1)
  PROOT_CPU=${PROOT_CPU:-0}
  ...
  echo "$TS,$MEM_TOTAL,$MEM_FREE,$MEM_CACHED,$MEM_AVAIL,$SWAP_USED,$OC_GW_RSS,$OC_PARENT_RSS,$PROOT_CPU,$GW_CPU,$DISK_FREE,$BAT_PCT,$BAT_TEMP" >> "$LOGFILE"
